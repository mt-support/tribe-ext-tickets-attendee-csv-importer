...
class Post_Repository extends Tribe__Repository {
	 ...
	 * @var string
	 */
	protected $filter_name = 'attendee_csv_importer_post';
	...
	public function filter_by_cost( $value, $operator = '=', $symbol = null ) {
		...
		$this->join_clause( "JOIN {$wpdb->postmeta} {$prefix}_ticket_event 
			ON ( 
		...
		$this->join_clause( $wpdb->prepare( "LEFT JOIN {$wpdb->postmeta} {$prefix}_ticket_cost 
			ON (
		...
		$prepared_value = is_array( $value ) ? $this->prepare_interval( $value, '%d', $operator ) : $wpdb->prepare( '%d', $value );

		// Default the cost to `0` if not set to make RSVP tickets show as "free" tickets, with a cost of 0.
		$this->where_clause( "IFNULL( {$prefix}_ticket_cost.meta_value, 0 ) {$operator} {$prepared_value}" );
	...
	public function filter_by_has_tickets( $has_tickets = true ) {
		...
		if ( (bool) $has_tickets ) {
			...
			$this->join_clause( "JOIN {$wpdb->postmeta} {$prefix}_ticket_event ON ( 
					{$prefix}_ticket_event.meta_value = {$wpdb->posts}.ID
